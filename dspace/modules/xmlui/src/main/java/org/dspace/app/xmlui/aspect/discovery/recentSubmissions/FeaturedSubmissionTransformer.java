 ...
 */
public class FeaturedSubmissionTransformer extends AbstractDSpaceTransformer {
    ...
    protected static final Message T_head = message("xmlui.Discovery.FeauturedSubmissions.FeaturedSubmissionTransformer.head");
    protected static final Message T_featured_submission_head = message("xmlui.Discovery.FeaturedSubmissions.FeaturedSubmissionTransformer.recent.head");
    protected static final Message T_trail = message("xmlui.Discovery.FeaturedSubmissions.FeaturedSubmissionTransformer.trail");
    ...
    public void addPageMeta(PageMeta pageMeta) throws SAXException, WingException, UIException, SQLException, IOException, AuthorizeException {
        ...
        if (name == null || name.length() == 0)
        {
            if(isHomePage){
                ...
                titlePageMeta.addContent(T_featured_submission_head.parameterize(name));
            }
        }
        else
        {
    ...
    public void addBody(Body body) throws SAXException, WingException, SQLException, IOException, AuthorizeException, ProcessingException {
        ...
        Division mainDivision = body.addDivision("main-featured-submissions");
        setMainTitle(dso, mainDivision);
        Division featuredSubmissionDivision = mainDivision.addDivision("featured-submissions");
        if(isHomePage){
            featuredSubmissionDivision.setHead(T_featured_submission_head);
        }

        DiscoverResult featuredSubmittedItems = FeaturedSubmissionUtils.getFeaturedSubmittedItems(context, dso, getOffset(request));
        setPagination(request, dso, featuredSubmissionDivision, featuredSubmittedItems);

        ReferenceSet lastFeatured = featuredSubmissionDivision.addReferenceSet(
                "last-featured", ReferenceSet.TYPE_SUMMARY_LIST,
                null, "featured-submissions");

        for (DSpaceObject resultObject : featuredSubmittedItems.getDspaceObjects()) {
            if(resultObject != null){
                lastFeatured.addReference(resultObject);
            }
        }
    }

    protected void setPagination(Request request, DSpaceObject dso, Division mainDivision, DiscoverResult featuredSubmittedItems) throws SQLException {
        int offset = getOffset(request);
        int rpp = FeaturedSubmissionUtils.getFeaturedSubmissionConfiguration(dso).getMax();
        int firstIndex = offset + 1;
        int lastIndex = offset + featuredSubmittedItems.getDspaceObjects().size();
        mainDivision.setSimplePagination((int) featuredSubmittedItems.getTotalSearchResults(), firstIndex,
                lastIndex, getPreviousPageURL(dso, offset, rpp), getNextPageURL(dso, offset, rpp, (int) featuredSubmittedItems.getTotalSearchResults()));
    }
