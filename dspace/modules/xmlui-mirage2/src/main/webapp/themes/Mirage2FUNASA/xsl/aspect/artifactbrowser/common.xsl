...
-->
    ...
    <!-- BIREME customization - Generate the logo, if present, from the file section -->
    <!-- xsl:template match="mets:fileGrp[@USE='LOGO']" -->
    <xsl:template match="mets:fileGrp[@USE='LOGO']" mode="second-level">
        <div class="ds-logo-wrapper">
            <img src="{mets:file/mets:FLocat[@LOCTYPE='URL']/@xlink:href}" class="logo img-responsive" style="border-radius:20px">
                <xsl:attribute name="alt">xmlui.dri2xhtml.METS-1.0.collection-logo-alt</xsl:attribute>
                <xsl:attribute name="attr" namespace="http://apache.org/cocoon/i18n/2.1">alt</xsl:attribute>
            </img>
        </div>
