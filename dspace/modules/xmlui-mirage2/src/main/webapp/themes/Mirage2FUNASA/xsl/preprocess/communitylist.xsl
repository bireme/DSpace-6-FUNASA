...
<xsl:stylesheet
    ...
    <xsl:template match="dri:referenceSet[@id='aspect.artifactbrowser.CommunityBrowser.referenceSet.community-browser']">
      <xsl:choose>
       <xsl:when test="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='request' and @qualifier='URI']='community-list'">
      ...
      <div>
            ...
            <xsl:text>row</xsl:text> 
              ...
              <xsl:apply-templates mode="community-browser-home"/>
        </div>
    ...
    <xsl:template match="dri:reference" mode="community-browser-home">
        <xsl:variable name="handle">
            <xsl:call-template name="get-handle-class-from-url">
                <xsl:with-param name="url" select="@url"/>
            </xsl:call-template>
              ...
              <xsl:apply-templates select="document($externalMetadataURL)" mode="community-browser-home" />
     ...
     <xsl:template match="mets:fileGrp[@USE='LOGO']" mode="thumb">
            <figure rend="mythumb" source="{mets:file/mets:FLocat[@LOCTYPE='URL']/@xlink:href}" />
     </xsl:template>
    
     <xsl:template match="mets:METS" mode="community-browser-home">
        <xsl:variable name="dim" select="mets:dmdSec/mets:mdWrap/mets:xmlData/dim:dim"/>
        
        <!--<xsl:apply-templates select="//mets:fileGrp[@USE='LOGO']" mode="thumb"/> -->
        <xref target="{@OBJID}">
         ...
         <div>
            ...
            <div>
              <xsl:attribute name="rend">
              <xsl:text>thumbnail boxcomm</xsl:text> 
              </xsl:attribute>
              
              <figure rend="mythumb" source="{//mets:fileGrp[@USE='LOGO']/mets:file/mets:FLocat[@LOCTYPE='URL']/@xlink:href}" />
       
              <!-- <figure rend="imgColecao" source="{//mets:fileGrp[@USE='LOGO']/mets:file/mets:FLocat[@LOCTYPE='URL']/@xlink:href}" /> -->
       
              <div>
                <xsl:attribute name="rend"><xsl:text>caption</xsl:text></xsl:attribute>
                <p>
                  <xsl:attribute name="rend"><xsl:text>linkhome</xsl:text></xsl:attribute>
                  <xsl:value-of select="substring($dim/dim:field[@element='title'],4,65)"/>
                </p>        
    ...
    <xsl:template match="//dri:div[@id='aspect.artifactbrowser.CommunityBrowser.div.comunity-browser']/dri:head">
      
          <xsl:if test="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='request' and @qualifier='URI']='community-list'">
            <head>  
            <xsl:apply-templates />
            </head>
          </xsl:if>
    ...
    <xsl:template match="//dri:div[@id='aspect.artifactbrowser.CommunityBrowser.div.comunity-browser']/dri:p">
       <xsl:choose>
          ...
          <xsl:when test="not(/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='request' and @qualifier='URI']='community-list')">
          <!-- <div rend="container">
            <div rend="row"> -->
              <div rend="col-md-12 headercomm">
                <figure rend="iconeComunidades" source="themes/Mirage2FUNASA/images/simboloComunidades.svg" />
                <p rend="headercommfont">Comunidades do repositório</p>
              </div>
            <!-- </div>
          </div> -->
          </xsl:when>
