...
function DSpaceChoiceLookupDeCS(url, field, formID, valueInput, authInput,
                            confIndicatorID, collectionID, isName, isRepeating, language) {
    ...
    url += '?field=' + field + '&formID=' + formID + '&valueInput=' + valueInput +
            '&authorityInput=' + authInput + '&collection=' + collectionID +
            '&isName=' + isName + '&isRepeating=' + isRepeating + '&confIndicatorID=' + confIndicatorID +
            '&limit=50&localeDeCS='+language; //limit to 50 results at once (make configurable?)
...
function DSpaceChoicesLoad(form) {
    ...
    var localeDeCS = $("*[name = localeDeCS]").val();    
    ...
    $.ajax({
        ...
        data: {query: value, collection: collID,
                     start: start, limit: limit, localeDeCS: localeDeCS},
        error: function() {
